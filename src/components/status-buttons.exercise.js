...
function StatusButtons({user, book}) {
  ...
  const { data } = useQuery({
    queryKey: 'list-items',
    queryFn: key => client('list-items', {token: user.token}),
  })

  // 🐨 search through the listItems you got from react-query and find the
  // one with the right bookId.
  const listItem = data ? data.listItems.find(item => item.bookId === book.id) : undefined
  ...
  const [update] = useMutation(
    (data) => {
      ...
      return client(`list-items/${data.id}`, {method: 'PUT', data, token: user.token})
    },
  ...
  const [remove] = useMutation(
    (data) => {
      ...
      return client(`list-items/${data.id}`, {method: 'DELETE', token: user.token})
    },
    {onSettled: () => queryCache.invalidateQueries('list-items')},
  )
  ...
  const [create] = useMutation(
    (data) => {
      ...
      return client(`list-items`, {data: {bookId: data.id}, token: user.token})
    },
    {onSettled: () => queryCache.invalidateQueries('list-items')},
  )
